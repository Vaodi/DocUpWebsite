...
const UpdateScreen = ({ secTitleWrapper, secText, secHeading }) => {
  return (
    <SectionWrapper id='testimonial'>
      ...
      <Container>
        <Box {...secTitleWrapper}>
          <Text {...secText} content="Comment ça fonctionne ?" />
          <Heading {...secHeading} content="Une application SIMPLE" />
        </Box>
        <Tabs
        ...
        >
          {SCREENSHOTS.map((item, index) => (
            <TabPane
              ...
              key={index + 1}
            >
              <div style={{ marginBottom: '40px', color: 'white', fontFamily: 'Heebo', fontWeight: 'bold', fontSize: '17px', textAlign: 'center', lineHeight: '2' }}>
